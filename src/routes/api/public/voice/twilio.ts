/**
 * Phase 2B — public Twilio voice webhook.
 *
 * Security boundary for this channel:
 ...
 * dialled number, and the customer from the authenticated call session.
 */
...
/** Exported for tests: runs the whole webhook without a live HTTP server. */
...
export const Route = createFileRoute("/api/public/voice/twilio")({
  server: {
    handlers: {
      POST: async ({ request }) => handleTwilioWebhook(request),
