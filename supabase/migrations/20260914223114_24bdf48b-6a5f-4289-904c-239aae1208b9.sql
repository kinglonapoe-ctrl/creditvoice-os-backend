...
revoke all on function public.cv_test_cleanup(uuid) from public, anon, authenticated;
do $$
begin
  if exists (select 1 from pg_roles where rolname = 'sandbox_exec') then
    execute 'grant execute on function public.cv_test_cleanup(uuid) to sandbox_exec';
  end if;
end $$;